...
## 2026-08-21 (Fri) — Pre-Market Routine

**Decision: HOLD — no new trades.** Regime ON but marginal (deterministic Alpaca SPY 762.62 > 20d SMA 760.94, only **+0.22%**; Gemini corroborates SPY 769.06 > 759.77 and S&P 7,641 > 7,631). No candidate clears both the 11-check hard gate AND the 3–8% entry-timing band — the persistent condition of recent weeks: the two names with a *fresh breakout on ≥1.5x volume* (PSX Energy, MRK Health Care) are both pinned near their highs at a **2.8% pullback** (just below the 3% floor → entry-timing FAIL = chasing); the names with real pullbacks (JPM, ROST) have **stale/rolled-over breakouts** (no fresh high in the last 5 sessions → c9 FAIL). Hold AMZN + PLTR under standard sell-side rules — both healthy, theses intact, stop-protected, no earnings in window. ⏳ **AMZN time stop is imminent — day 14 of 15 (entered 8/04), fires at Monday 8/24's scan** if still <+5%; not yet triggered today. Event-heavy week ahead (Jackson Hole 8/27–29, PCE Wed 8/26, NVDA earnings Wed 8/26) argues against forcing a marginal entry into a thin-margin regime.

### Market Regime Status — ON (marginal, +0.22%)
- **Alpaca (deterministic, `buy_gate.sh`):** SPY last close **762.62** vs **20-day SMA 760.94** → **REGIME ON**, but a thin **+0.22%** gap (83 bars). No flip vs the last committed read (8/17: ON). New longs permitted, but each candidate must still clear the full 11-check gate AND the entry-timing rule.
- **Gemini corroborated:** S&P 500 closed **7,641.16** vs 20-day SMA **7,631.31** (above); SPY **769.06** vs SMA **759.77** (above). Both sources agree → regime ON. (The two feeds' absolute SPY levels differ — IEX free-tier vs consolidated — but the sign is the same.)

### Account Snapshot (live Alpaca, paper)
Equity: **$99,288.36** | Cash: **$94,482.64** | Buying power: $391,386.58 | Positions: **2 (AMZN, PLTR)** | Day trades: 0/3 | PDT: false
- last_equity $99,259.07 (balance_asof 8/20) → true session move ≈ **+$29.29 (+0.03%)**, essentially flat pre-bell; exactly matches the two positions' intraday P&L (AMZN +$13.05 + PLTR +$16.24 = +$29.29).
- Phase P&L **-$711.64 (-0.71%)** off the live $100k base.
- **Trades this week: 0/3 (week from Mon 8/17).** Room for 3, but patience beats activity.

### Market Context
- **VIX ~15.8** (closed 15.82, -1.19% d/d) — benign / risk-on, below its ~18.6 long-run average; nudged up from the 8/17 YTD low (~14.2) as geopolitical risk crept in, but still contained (<20).
- **Macro calendar:** Today (Fri 8/21) = **prelim Aug S&P Global manufacturing & services PMIs** only. This week already cleared: FOMC minutes (Wed 8/19), jobless claims 206k (Thu 8/20), Philly Fed jumped to 47.4. **Next week is event-heavy: Jackson Hole Symposium 8/27–29 (Chair Warsh keynote Fri 8/28 — his first), July PCE + Q2 GDP 2nd estimate Wed 8/26, jobless claims Thu 8/27.** Next FOMC 9/16.
- **10-yr Treasury ~4.70%, 30-yr ~5.25%** — yields firming, a mild headwind for rate-sensitive momentum.
- **WTI crude ~$86/bbl** (closed $86.07, -0.74% d/d) — up on the week (2nd straight weekly gain), highest since late July, driven by **US–Iran tensions / Middle East supply-disruption fears**. Energy is the standout strength.

### Sector Momentum
- **Leading (week to 8/14):** Information Technology **+7.5%** and Consumer Discretionary **+5.2%** led, then Financials +3.2%, Materials +2.2%, Industrials +2.1%.
- **Thu 8/20 risk-off rotation:** the tape closed lower on **US–Iran geopolitical concerns** — Energy the only sector up (+0.4%); Consumer Staples, Health Care, and Consumer Discretionary each fell ~1.8–1.9%. Fri futures modestly positive (Nasdaq +0.4%, S&P/Dow +0.3%, Russell +0.5%).
- **Idea pool (fresh-high leaders):** MRK (Health Care, ran to a $153.4 high), PSX (Energy, on oil strength), JPM (Financials, 17 ATHs YTD but rolling), plus crypto-proxy movers (MSTR/MARA/COIN on BTC >$74k — not tradeable under our stocks-only, breakout-pullback rules) and ROST (+8% earnings gap).

### Held-Position Review
- **AMZN** (Consumer Discretionary, 9 sh @ $276.95) — current **$261.56**, **-5.56%** (unrealized -$138.51; cost basis $2,492.55, mkt value $2,354.04; intraday +$13.05 today). Above the -8% cut and below the +15% tighten threshold → **no price-based sell-side rule triggered.** Live 12% trailing-stop GTC intact (id **26477093**, stop **$248.85**, hwm $282.78; qty_available 0 = reserved). Q2 reported 7/30 (rev +20% to $200.6B, AWS +37% to $42.2B, backlog $496B; topped $3T cap 8/03); **next earnings ~10/22–11/02/2026** — outside the 10-day window. News benign: $220B 2026 capex overhang (mild FCF concern), NJ antitrust suit, routine insider selling; consensus overwhelmingly Buy/Strong-Buy. ⏳ **Time stop imminent:** entered ~8/04 → **14 of 15 trading days held** at a <+5% gain (in fact -5.56%); the 15-day time stop **fires at Monday 8/24's pre-market/midday scan** if it hasn't turned to a gain — NOT triggered today (day 14). **HOLD today; queue the time-stop decision for Mon 8/24.**
  ⚠️ **Undocumented entry (still open item):** AMZN has no committed research/trade-log buy entry — the recurring pattern flagged for CVX (6/12), JPM (~7/01), MS/UNH (7/10–7/20). It IS fully stop-protected. Operator should confirm the actual fill price/date/catalyst.
- **PLTR** (Technology, 14 sh @ $173.70) — current **$175.12**, **+0.82%** (unrealized +$19.83; cost basis $2,431.85, mkt value $2,451.68; intraday +$16.24). Below the +15% tighten threshold → **no stop change yet.** Live 12% trailing-stop GTC intact (id **c89fba0d**, stop **$158.54**, hwm $180.16; qty_available 0 = reserved). Bought 8/13 on a blowout Q2 (rev +93% y/y, US-commercial +149%, FY guide raised to ~$8.15B); consolidating below $179 near recent highs, thesis intact; **next earnings ~90d out** — outside the window. Entered 8/13 → **7 trading days held** (inside the 15-day time stop). News: valuation-caution noise only (high forward multiples, routine 10b5-1 insider selling) — not a thesis break. **HOLD** — let it work; watch the +15% level to tighten to 7%.

### Trade Ideas (regime ON) — deterministic `buy_gate.sh` breakout/volume/pullback screen
Screened the week's fresh-high leaders. *(IEX free-tier feed understates consolidated volume, so c10 ratios are conservative; breakout-recency and pullback-timing — the robust checks — decide these.)* **No name clears the hard gate AND entry timing simultaneously.**

- **$PSX** (Energy) — **hard gate PASS.** Fresh 3-mo high **$246.86** within last 5d (c9 PASS) on **~1.24x** 20-day volume by the conservative IEX max-bar (last5_vol_max 269,225 > threshold 216,769 → **c10 PASS**); Energy held 0 → no crowding; next earnings outside window. ❌ **Entry-timing FAIL:** pullback from the high is only **2.8%** — just under the 3% floor; PSX is pinned to its breakout high on the oil bid. Buying now = chasing. **Watchlist — wait for a valid 3–8% pullback that holds support.**
- **$MRK** (Health Care) — **hard gate PASS.** Fresh high **$153.41** within last 5d (c9 PASS) on heavy **~3.6x** volume (last5_vol_max 1,348,714 > 565,552 → **c10 PASS**); it has run ~10% off last week's ~$135 breakout. ❌ **Entry-timing FAIL:** **2.8%** pullback — pinned near the high, extended after a sharp run. **Watchlist.**
- **$JPM** (Financials) — **c9 breakout FAIL** (last5_high 365.82 < prior_high 366.30 → no fresh high in the last 5 sessions, the breakout has rolled over) **and c10 volume FAIL** (200,442 < 299,811), despite an in-band 3.8% pullback. Dropped.
- **$ROST** (Consumer Discretionary) — gapped **+8% on an earnings beat/raised outlook today** → it's a post-earnings gap, not a clean first-pullback setup; **c9 breakout FAIL** (last5_high 246.8 < prior 256.64). Dropped.
- **$MRVL** (Technology) — **c9 breakout FAIL** (last5_high 251.44 << prior_high 329.85 — trading well below its high, no fresh breakout); 0.1% "pullback" is just noise near a local level. Dropped.

### STOCK Act scan (idea-generation only — never a buy reason)
**Sen. John Boozman (R)** disclosed a small **PYPL** purchase ($1k–$15k, trade 7/2, filed 8/17) and a **SPYM** (2x-leveraged SPY ETF) purchase — the latter a leveraged ETF we never trade; PYPL is a stale, small-size buy and not a fresh-breakout leader. Assorted senators flagged for *late* filings (no purchase detail). **Nothing actionable** — disclosures lag up to ~45 days, omit size/exits, and each name must independently clear the full gate. Dropped — no gate impact.

### Risk Factors
- **AMZN time stop fires Monday.** It's a losing position (-5.56%) drifting on day 14 of 15 at a <+5% gain; barring a sharp turn it becomes a time-stop close at Mon 8/24's scan. Its live 12% trailing stop ($248.85) protects it in the meantime, but the clock, not hope, decides it.
- **Marginal regime + event-heavy week.** The regime is ON by only +0.22% on the deterministic feed; next week stacks Jackson Hole (Warsh 8/28), PCE (8/26), and NVDA earnings (8/26). A hawkish Warsh or hot PCE could flip the thin regime OFF. Firming yields (10-yr ~4.70%) and Thursday's US–Iran risk-off add two-sided event risk. Forcing a 2.8%-pullback leader into this is exactly the low-quality entry the rules prevent.
- **Leaders pinned to highs.** Both fresh-breakout names (PSX/MRK) sit near their highs with no valid 3–8% pullback; the tradeable-pullback names have stale breakouts. The risk today is chasing/overpaying, not missing out. Force nothing.

### Decision
**HOLD — no new trades.** Regime ON but marginal (SPY +0.22% deterministic; Gemini corroborates), VIX ~15.8 risk-on, yet no candidate clears both the 11-check hard gate and the entry-timing rule: PSX and MRK have fresh breakouts on volume but sit at 2.8% pullbacks (pinned near highs → chasing), while JPM/ROST/MRVL have stale or rolled-over breakouts. Watchlist: PSX/MRK for a valid 3–8% pullback. Hold **AMZN** (-5.56%, stop $248.85, next earnings ~10/22+, **time stop day 14/15 → fires Mon 8/24**) and **PLTR** (+0.82%, stop $158.54, 7 days held) under standard sell-side rules with their live 12% trailing stops. Trades this week 0/3. No urgent notification warranted (AMZN -5.56% >> -8% cut, no earnings within 3 days for either holding, regime unchanged ON with no flip — silent per STEP 5). Committed and pushed per the pre-market routine.
