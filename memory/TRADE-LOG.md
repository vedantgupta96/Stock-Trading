...
Equity: $99,681.74 | Cash: $99,681.74 | Day P&L: -$7.47 (-0.01%) | Phase P&L: -$318.26 (-0.32%)
Trades today: 0 documented | Trades this week: 0/3 new | Regime: ON (last read 6/22 — STALE, no fresh research since)

| Symbol | Shares | Entry | Current | Unreal. P&L | Stop type |
|--------|--------|-------|---------|-------------|-----------|
| —      | —      | —     | —       | —           | —         |

Notes: ⚠️ UNDOCUMENTED EXIT — the portfolio is back to 100% cash: JPM (the lone position on the 6/26 snapshot, 6 sh @ $327.17) is gone, along with its 12% trailing-stop GTC ($302.23, hwm $343.45), and there is NO committed research/trade-log entry recording the close. Reconstructing from cash ($97,718.69 on 6/26 → $99,681.74 now, +$1,963.05 / 6 sh) implies an exit at ~$327.18 — essentially breakeven (realized ~$0.00). That price sits far ABOVE JPM's $302.23 trailing stop, so this was NOT a stop fill — it was a discretionary/manual close near entry, executed on a session (likely 6/29 or earlier) whose snapshot was missed. JPM now trades $311.69 (close bid), so the exit was well above current price — the close avoided a subsequent ~5% drawdown and sidestepped the binding 7/13 earnings-exit deadline. This is the second undocumented discretionary exit (CVX was the first); operator should confirm the actual fill/date/reason via Alpaca order history. Equity $99,681.74, down -$7.47 (-0.01%) vs the last committed EOD (6/26, $99,689.21); true single-session move vs Alpaca last_equity (6/29, $99,695.03) ≈ -$13.29 (-0.01%) — flat. Phase P&L -$318.26 (-0.32%) off the live $100k base. Day-trade count 0/3, PDT false; zero open positions, zero open orders. Regime last read ON (6/22, SPY 749 > 20d SMA 747) but that read is STALE — no fresh research ran today (EOD-only routine). Outlook: fully in cash and flat; run a fresh pre-market regime/research check and confirm the JPM exit details before any new entries.
